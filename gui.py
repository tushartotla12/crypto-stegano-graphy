import os, crypto, stego, smtplib
import email.utils
from mail import *
...
tab1_layout = [
    [
        sg.Column([ 
            ...
            [sg.Column([[sg.Input(key='image_file', disabled_readonly_background_color='black'), sg.FileBrowse(key='image_file_browse', file_types=(('PNG Files', '*.jpg'), ))]], key='image_file_row')], 
...
tab2_layout = [
    [
        sg.Column([
            ...
            [sg.Text(text="Key", key='private_key_title')], 
...
tab4_layout = [
    [
        sg.Column([
            [sg.Text(text="Encrypted file", key='mailfile_title')], 
            [sg.Column([[sg.Input(key='mail_file'), sg.FileBrowse(key='mail_browse')]], key='mail_row')], 
            [sg.Text(text='', key='mail_message', size=(40, 2))], 
            [sg.Text("Your Email Address")], 
            [sg.Input(key='sender_addr')], 
            [sg.Text("Password")], 
            [sg.Input(key='mail_pwd', password_char='*')], 
            [sg.Text("Receiver Email Address")], 
            [sg.Input(key='receiver_addr')], 
            [sg.Button(button_text="Send Mail", key='send_mail')]
        ], vertical_alignment='top')
    ]
]
...
layout = [
    [
        sg.TabGroup([
            [sg.Tab("Encrypt", tab1_layout), sg.Tab("Decrypt", tab2_layout), sg.Tab("Keys", tab3_layout), sg.Tab("Mail", tab4_layout)]
        ])
    ]
]

window = sg.Window("CRYPEGANgraphy", layout)
while True:
    window.refresh()
    ...
    if event == sg.WIN_CLOSED:
        ...
        window['mail_message'].update("Successfully sent mail", text_color='green')
